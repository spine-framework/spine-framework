...
# 6. Copy custom/apps/ → .assembled/custom/apps/ (preserves path for CustomAppLoader glob)
ASSEMBLED_CUSTOM_DIR="$PROJECT_ROOT/.assembled/custom/apps"
if [ -d "$CUSTOM_SRC_DIR" ]; then
  mkdir -p "$ASSEMBLED_CUSTOM_DIR"
  cp -r "$CUSTOM_SRC_DIR"/* "$ASSEMBLED_CUSTOM_DIR"/ 2>/dev/null || true
