...
// Extract slug from a glob key like '../../custom/apps/cortex/index.tsx' → 'cortex'
function slugFromKey(key: string): string {
  return key.split('/').slice(-2)[0]
}

/**
 * Returns the set of app slugs that are physically present in this build.
 * Used by App.tsx to filter DB app records down to only what's actually installed,
 * preventing route registration for apps that exist in the DB but not this deployment.
 */
export function getInstalledAppSlugs(): Set<string> {
  const slugs = new Set<string>()
  Object.keys(customAppModules).forEach(k => slugs.add(slugFromKey(k)))
  Object.keys(coreAppModules).forEach(k => slugs.add(slugFromKey(k)))
  return slugs
}
